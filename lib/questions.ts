...
// This array will hold all 350 questions across 7 exam sets (50 questions each)
// Questions are organized sequentially: Set 1 (questions 0-49), Set 2 (50-99), etc.
// 
// STRUCTURE FOR 350 QUESTIONS ACROSS 7 SETS:
// SET 1: Array indices 0-49    (Questions 1-50)
// SET 2: Array indices 50-99   (Questions 51-100)  
// SET 3: Array indices 100-149 (Questions 101-150)
// SET 4: Array indices 150-199 (Questions 151-200)
// SET 5: Array indices 200-249 (Questions 201-250)
// SET 6: Array indices 250-299 (Questions 251-300)
// SET 7: Array indices 300-349 (Questions 301-350)
//
// Each set will have exactly 50 questions for balanced exam distribution
...
// ===== ENHANCED QUESTION ACCESS FUNCTIONS FOR 7 EXAM SETS =====
...
export function getUniqueExamSet(examSetNumber: number): ExamQuestion[] {
  ...
  // Ensure we have enough questions for this set
  if (QUESTIONS.length < endIndex) {
    console.warn(`Warning: Exam set ${examSetNumber} requires questions ${startIndex + 1}-${endIndex}, but only ${QUESTIONS.length} questions available.`)
    // Return available questions if not enough for full set
    return QUESTIONS.slice(startIndex, QUESTIONS.length)
  }
...
// Get random questions from any of the available exam sets
export function getRandomExamQuestions(count: number = 50): ExamQuestion[] {
  const availableSets = Math.floor(QUESTIONS.length / 50)

  if (availableSets === 0) {
    console.warn('Not enough questions for any complete exam set')
    return QUESTIONS.slice(0, Math.min(count, QUESTIONS.length))
  }

  // Randomly select one of the available exam sets
  const randomSet = Math.floor(Math.random() * availableSets) + 1
...
// ===== UTILITY FUNCTIONS FOR EXAM SET MANAGEMENT =====

// Get comprehensive exam structure information
export function getExamStructureInfo() {
  const totalQuestions = QUESTIONS.length
  const expectedSets = 7
  const questionsPerSet = 50
  const expectedTotal = expectedSets * questionsPerSet
  const availableSets = Math.floor(totalQuestions / questionsPerSet)
  const incompleteSetQuestions = totalQuestions % questionsPerSet
  ...
  return {
    ...
    expectedTotal,
    availableSets,
    incompleteSetQuestions,
    isComplete: totalQuestions >= expectedTotal,
    completionPercentage: Math.round((totalQuestions / expectedTotal) * 100)
  }
}

// Get all available complete exam sets
export function getAllAvailableExamSets(): ExamQuestion[][] {
  const info = getExamStructureInfo()
  const sets: ExamQuestion[][] = []

  for (let i = 1; i <= info.availableSets; i++) {
    sets.push(getUniqueExamSet(i))
  }

  return sets
}

// Validate that the exam structure meets requirements
export function validateExamStructure(): {
  valid: boolean;
  message: string;
  details: ReturnType<typeof getExamStructureInfo>
} {
  const info = getExamStructureInfo()

  if (info.totalQuestions < info.expectedTotal) {
    return {
      valid: false,
      message: `Need ${info.expectedTotal} questions (7 sets × 50), currently have ${info.totalQuestions} (${info.completionPercentage}% complete)`,
      details: info
    }
  }
  ...
  if (info.totalQuestions === info.expectedTotal) {
    return {
      valid: true,
      message: `Perfect! Exam structure complete: ${info.availableSets} sets of ${info.questionsPerSet} questions each`,
      details: info
    }
  }
  ...
  return {
    valid: true,
    message: `Exam structure valid: ${info.availableSets} complete sets (${info.incompleteSetQuestions} extra questions available)`,
    details: info
  }
}

// Get topic distribution across all questions
...
// Generate a summary report of the question database
export function generateQuestionDatabaseSummary() {
  const structure = getExamStructureInfo()
  const validation = validateExamStructure()
  const topics = getTopicDistribution()
  ...
  return {
    ...
    topics,
    sets: {
      available: structure.availableSets,
      complete: structure.availableSets === structure.expectedSets,
      missingQuestions: Math.max(0, structure.expectedTotal - structure.totalQuestions)
    }
  }
}